...
auto compute_file_size(const std::filesystem::path & path)
{
	...
	{
		auto file_size = std::filesystem::file_size(path); //запрашиваем размер файла
...
auto compute_directory_size(const std::filesystem::path & path) //суммарный объем памяти занимаемый дир
{
	uintmax_t size = 0ULL;
...
void view_directory(const std::filesystem::path & path) //обход директории просмотр .//вывод типа, время последнего измерения ласт рантайм
{
	if (std::filesystem::exists(path) && std::filesystem::is_directory(path))
	{
